...
class StormEclSupport:
	...
	def eraseAll():
		...
		toErase += [files for files in glob.glob('*.DATA')]
		toErase += [files for files in glob.glob('*.txt')]
		toErase += [files for files in glob.glob('*.INC') if 'SCH' in files]
		for files in toErase: os.remove(files)
