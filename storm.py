...
class StormBackend:
	...
	def __init__(self,caseName,**kwargs):
		...
		self.phases = (1,1,1) 				# Phases: oil, water, gas
	...
	def __moveIters(self):
		...
		Returns:
			None.
		"""
		iterFiles = [i for i in glob.glob('*.DATA')]
		iterFiles.extend([i for i in glob.glob('*.INC') if 'SCH' in i])		# Gets all the iteration case files and their correspondent schedules.
		for name in iterFiles:
			subprocess.call(['mv',name,'{}'.format(self.dumpName)])		# Move the files to the dump folder.
	...
	def MNPOIteration(self,verbosities=(True,'false')):
		...
		for i in range(_n):																									# For each possibility in span...
			...
			self.execFlow(iterFile, verbosities[1])																			# Execute OPM Flow
...
def caseConf():
	...
	wp,wi = ['PROD-%d'%i for i in range(1,12)],['INJ-%d'%i for i in [1,2,3,4,5,6,7]]	# Well names
	...
	k = StormBackend('OLYMPUS',**caseConfigurations) # Case initialization
...
def mnpoConf():
	...
	k = StormBackend('OLYMPUS',**caseConfigurations)	# Case init
